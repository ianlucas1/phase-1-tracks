...
describe DragonFly do
  ...
  describe "attacking and defending" do
    ...
    it "is not immune to other types of attacks" do
      expect(mascot.immune_to?(:water)).to be false
    end

    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:silver_wind, :pin_missile, :bug_bite, :buzz]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:silver_wind)).to be true
      end
